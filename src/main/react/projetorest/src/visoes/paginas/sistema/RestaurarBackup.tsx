...
import {realizarBackup, restaurarBackup} from "../../../servicos/sistema.servico";

interface RestaurarBackupProps {
    backup?: string;
}

const RestaurarBackup: FC<FormularioProps> = props => {
    const {usuarioLogado, setSelectedItem} = props;
    const {handleSubmit, register, control} = useForm<RestaurarBackupProps>();
    const history = useHistory();
    const { addToast } = useToasts();
    const onSubmit = () => {
        const backup = control.getValues("backup");
